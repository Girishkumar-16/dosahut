      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 md:px-8">
        {query.trim() === "" ? (
          ...
          <>
            ...
            {/* Same rail-on-mobile, grid-on-desktop shape as the Craving
                Finder, so results behave identically wherever they appear. */}
            <div className="scrollbar-none -mx-4 flex w-[calc(100%+2rem)] snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-4 md:mx-0 md:grid md:w-full md:grid-cols-2 md:overflow-visible md:px-0 lg:grid-cols-3 xl:grid-cols-4">
              {results.map((item) => (
                <div
                  key={item.code}
                  className="min-w-[85vw] flex-shrink-0 snap-center sm:min-w-[340px] md:min-w-0"
                >
                  <DishCard item={item} />
                </div>
              ))}
